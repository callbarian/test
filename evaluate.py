...
class GroundTruthLoader(object):
    ...
    UCFCRIME = 'ucfcrime'
    UCF_CRIME_LABEL_PATH = os.path.join(DATA_DIR,'ucf_crime/test/gt')
    ...
    def __call__(self, dataset):
        ...
        elif dataset == GroundTruthLoader.UCFCRIME:
            gt = self.__load_ucf_crime_gt()
        else:
    ...
    def __load_ucf_crime_gt():
        video_path_list = os.listdir(GroundTruthLoader.UCF_CRIME_LABEL_PATH)
        
        video_path_list.sort()
        gt = []
        for video in video_path_list:
            gt.append(np.load(os.path.join(GroundTruthLoader.UCF_CRIME_LABEL_PATH, video)))
